...
def test_DataPointModel_with_valid_input(valid_raw_input):
    """Test if DataPointModel validation fails due to missing `filename` field."""
    with pytest.raises(pydantic.ValidationError):
        DataPointModel.parse_raw(valid_raw_input)
...
def test_DataPointModel_with_enriched_input(enriched_input):
    ...
    assert datapoint.dict() == dict(datapoint) == {"id": 1, "created_at": "date", "lang": "id", "text": "text", "filename": "/data/file.zip"}
