...
For detailed info about the data model, check the [DisasterModel & DisasterSchema](build/libdrm/src/libdrm/datamodels/disaster.py)
...
> This task required Docker Engine to be installed on you host.
...
The following command uses the [docker-compose.yml](docker-compose.yml) and [.env](.env) files to define
...
> :warning: WARNING :warning:
> This task required Docker Compose to be installed on you host.
...
> :warning: WARNING :warning:
> You need to run `bash build/image.sh` if you want to use that image for development.
...
DISASTER_TYPE=floods python -m pytest --disable-warnings build/libdrm
```
...
> :warning: WARNING :warning:
> Be aware that once you run this command the data in ElasticSearch will be deleted, and you need to reprocess
> the zip files to put it back.
